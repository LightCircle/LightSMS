...
var config    = light.framework.config
...
exports.send = function (option, callback) {
  ...
  request.post({
    url: constant.SMS_SEND_HUIYI, form: option, proxy: process.env.DEV ? undefined : config.app.proxy
    ...
    log.error({code: code.content, msg: msg.content});
